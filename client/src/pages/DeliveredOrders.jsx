...
const DeliveredOrders = () => {
	...
	useEffect(() => {
		...
		if(!loading && isAdmin){
			loadAllDeliveredOrders({status:'delivered'});
		}
	}, [loading])

	const applyFilters = ()=>{
		let filterData = {status:'delivered'};
		...
		<div className='orders'>
			...
			<div className='container'>
				{!areOrdersFetched
					...
					?<Empty/>
					:deliveredOrders.map((product,ind)=>(
						<AllOrdersCard key={ind} product={product} type='delivered' />
					))
				}
			</div>
		</div>
